...
const ViewAdmissions = () => {
  ...
  useEffect(() => {
    const fetchAdmissions = async () => {
      try {
        const response = await fetch("https://arjun-academy-4c7g.onrender.com/admissions");
        if (response.ok) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this record?")) {
      try {
        const response = await fetch(`https://arjun-academy-4c7g.onrender.com/admissions/${id}`, {
          method: "DELETE",
        });
        if (response.ok) {
